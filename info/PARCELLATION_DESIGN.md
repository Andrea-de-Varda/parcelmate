...
**Both metrics get a ceiling, and the two partial out different nuisances.** The fidelity ceiling holds the data split fixed and removes the *model* limitation; the reliability ceiling holds the model fixed and removes the *data* difference. Same purpose -- how much of the shortfall is not the method's fault -- different mechanism.

| ceiling | construction | what is left over |
|---|---|---|
| fidelity | predict half B from half A directly, no parcellation | sampling noise between halves |
| reliability | two consensuses from disjoint halves of the *same* restarts, same data | algorithmic instability |

There is no literal analogue of "no parcellation" for reliability, because reliability compares two partitions and removing the parcellation leaves nothing to compare. The seed ceiling is the closest meaningful control, and it is necessary rather than optional for two reasons. First, seed variability cannot be neutralized by fixing the seed: with different data the same RNG stream produces an unrelated trajectory, so measuring it is the only option. Second, and decisively, the arms differ by nearly an order of magnitude in this floor -- `current` scores ARI 0.096 across seeds on identical wikitext, `nopca_fisher` scores 0.741 -- so raw cross-half reliability is **not comparable between arms** without dividing through by each arm's own ceiling. An arm could otherwise look more reliable purely by having a quieter algorithm. It costs two Hungarian alignments, measured at 0.1% of the k-means time.

**Hard argmax labels, and peakedness reported alongside.** The deliverable is a discrete parcellation, and reliability requires hard labels anyway, so both metrics then describe the same object. A genuinely soft fidelity would need soft block means as well, at which point it is a mixed-membership block model -- a different question rather than a variant of this one. But the viability of discretization turns out to be arm-dependent: `nopca_fisher` puts 51.6% of units above 0.5 membership (median max 0.53), while `current` puts 1.6% there (median max 0.20, against 0.05 for uniform), so its argmax is reading the top of a nearly flat noisy vector. `triviality` therefore reports `median_max_membership` and `frac_confident`, so a reader can see when a hard label is load-bearing and when it is an artifact of taking argmax of noise. Relatedly, for `current` the *soft* consensus agrees across seeds 4-5x better than its argmax does, so the instability lives in the discretization rather than in the underlying consensus.

**Raw values, not normalized ones.** The CSV holds one row per tree, variant, metric and domain pair, unnormalized. Deltas, ratios and ceiling-relative scores are computed downstream, so nothing is baked in and everything remains available for plotting.

**Fidelity's uncompressed reference in detail**: predict half B from half A directly, with no parcellation, every unit keeping its own identity. Whatever is unexplained there is sampling noise between halves, so it converts fidelity from an arbitrary number into a fraction of what was achievable. It is *not* a strict upper bound, and the exception is informative: the uncompressed predictor carries all of half A's noise, whereas a block model averages that noise away, so a parcellation can beat the reference when the structure genuinely is block-like and the estimates are noisy. This is demonstrated on synthetic data in `tests/verify_iter4_metrics.py`. On real connectivity, whose structure is far richer than any 50-block summary, the reference should sit above every variant; a variant exceeding it would be a finding, not a bug.
...
**Protocol choices**, settled 2026-09-09. Halves are Fisher-avg(samples 1,2) versus Fisher-avg(samples 3,4), ~197k tokens each. Block means are estimated on the fitting half, so fidelity is fully out of sample. Domains are weighted equally rather than by token count: the question is whether a method works across domains, and letting a large corpus dominate would answer a different one. Hard argmax labels throughout, settled 2026-09-09; see above for why, and for the peakedness diagnostic that accompanies them. Restarts are matched across arms at 20 -- matched deliberately, since comparing arms at different restart budgets would confound method with budget.

**Within versus across domain** applies to both metrics -- `reliability_across` asks whether the same partition reappears when the model reads different text, `fidelity_across` whether it still explains that text's connectivity. It is just which matrix is held out. Within: fit on wikitext half A, score wikitext half B. Across: fit on wikitext's average, score codeparrot's. The across/within ratio is a *continuous* measure of domain-generality, and a better one than the reciprocal-best-match clique of section 1, which is a binary survive-or-die test that discards how *much* a network transfers.
